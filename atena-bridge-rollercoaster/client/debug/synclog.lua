...
-- ROBUST: no permanent top-guard bail (load race). The loop reads exports['atena-std-rollercoaster'] only when
        ...
        if not Dbg.on.sync then
            ...
            if lead then
                Dbg.clog('cartpos', ('localProg=%.1f serverProg=%.1f cart1=(%.2f,%.2f,%.2f) phase=%s'):format(
                    exports['atena-std-rollercoaster']:localProg() or -1, (gs('coasterProg') or {})[1] or -1,
                    lead.x, lead.y, lead.z, gs('coasterPhase') or '?'))
            end
            if (gs('coasterPhase') or '') == 'running' then
                local localId = GetPlayerServerId(PlayerId())
                for srv, seat in pairs(exports['atena-std-rollercoaster']:attachedMap() or {}) do
                    if srv ~= localId then
                        local plyr = GetPlayerFromServerId(srv)
                        local ped = (plyr ~= -1) and GetPlayerPed(plyr) or 0
                        if ped ~= 0 then
                            local pp = GetEntityCoords(ped)
                            local car = exports['atena-std-rollercoaster']:carEntity(exports['atena-std-rollercoaster']:carOf(seat))
